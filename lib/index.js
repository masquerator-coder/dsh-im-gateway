...
function htmlToText(html) {
  return html.replace(/<(script|style|head)[\s\S]*?<\/\1\s*>/gi, " ").replace(/<\/?(p|div|br|tr|li|h[1-6]|table|section|article|blockquote)\b[^>]*>/gi, "\n").replace(/<[^>]*>/g, "").replace(/&nbsp;/gi, " ").replace(/&lt;/gi, "<").replace(/&gt;/gi, ">").replace(/&quot;/gi, '"').replace(/&#0*39;|&apos;/gi, "'").replace(/&amp;/gi, "&").replace(/[ \t]+/g, " ").replace(/\n\s*\n\s*\n+/g, "\n\n").split("\n").map((line) => line.trim()).join("\n").trim();
}
    ...
    WechatIlinkTransport = class {
      ...
      async saveState() {
        try {
          ...
          await writeFile2(this.stateFile, JSON.stringify(this.state), { encoding: "utf8", mode: 384 });
        } catch (error) {
...
var ChannelRecordSchema = z.object({
...
var InteractionBridge = class {
  ...
  requestApproval(sessionId, send2, req, next) {
    ...
    return new Promise((resolve2) => {
      const record = {
        kind: "approval",
        send: send2,
        tryAnswer: (text) => this.parseApprovalReply(text, resolve2),
        abort: () => resolve2("cancelled")
      };
      ...
      void this.sendPrompt(send2, prompt, sessionId, () => {
        this.delegateOnFailure(sessionId, record, () => {
          ...
          void Promise.resolve(next()).then(resolve2, () => resolve2("unavailable"));
        });
      });
    });
  }
  ...
  requestQuestion(sessionId, send2, req, next) {
    ...
    return new Promise((resolve2) => {
      const record = {
        kind: "question",
        send: send2,
        tryAnswer: (text) => this.parseQuestionReply(questions, text, resolve2),
        abort: () => resolve2({ answers: [] })
      };
      this.pending.set(sessionId, record);
      record.detach = this.attachAbort(req.signal, () => this.abortIfCurrent(sessionId, record));
      void this.sendPrompt(send2, prompt, sessionId, () => {
        this.delegateOnFailure(sessionId, record, () => {
          ...
          void Promise.resolve(next()).then(resolve2, () => resolve2({ answers: [] }));
        });
      });
    });
  }
  ...
  parseQuestionReply(questions, text, resolve2) {
    const t = text.trim();
    const answers = [];
    ...
    if (questions.length === 1 && /^[\d,\s]+$/.test(t)) {
      ...
      resolve2({ answers: [{ id: String(q.id), selected }] });
      return true;
    }
    const first = questions[0];
    if (first !== void 0) {
      resolve2({ answers: [{ id: String(first.id), selected: [], custom: t }] });
      return true;
    }
    return false;
  }
...
function timeout(ms) {
  return new Promise((resolve2) => {
    setTimeout(() => resolve2("timeout"), ms);
  });
}
function delay(ms) {
  return new Promise((resolve2) => {
  ...
  return new Promise((resolve2) => {
    ...
    req.on("data", (chunk) => {
      ...
      if (size > MAX_BODY_BYTES) {
        chunks.length = 0;
        req.destroy();
        resolve2({ ok: false, status: 413, error: "payload too large" });
        return;
      }
      chunks.push(chunk);
    });
    req.on("end", () => {
      ...
      if (raw.trim() === "") return resolve2({ ok: true, body: void 0 });
      try {
        resolve2({ ok: true, body: JSON.parse(raw) });
      } catch {
        resolve2({ ok: false, status: 400, error: "malformed JSON body" });
      }
    });
    req.on("error", () => resolve2({ ok: false, status: 400, error: "request aborted" }));
  });
}
...
var ChannelManager = class {
  ...
  attach(channels, cwd) {
    ...
    this.cwd = cwd;
    this.reconcile();
    const off = this.ctx.on("loader/volatile-update", () => {
      this.reconcile();
    });
    this.detachSettings = () => {
      off();
    ...
    const routeInbound = (route) => {
      ...
      void this.gateway.handle(
        ...
        },
        {
          ...
          allowlist: channel.allowlist === void 0 ? [] : [...channel.allowlist],
...
import { basename, dirname, isAbsolute, join as join5, resolve, sep } from "node:path";
function defaultBrowseDeps() {
  return {
    list: async (path) => {
    ...
    home: safeHomedir(),
    cwd: "",
    imWorkspace: join5(safeHomedir(), ".dsh", "im-workspace"),
...
function resolveBrowsePath(path, deps) {
...
async function browseDirectory(requested, deps) {
  ...
  for (const name2 of names) {
    const child = join5(path, name2);
    entries.push({ name: name2, path: child, readable: await isReadableDirectory(child, deps) });
  }
  entries.sort((a, b) => a.name.localeCompare(b.name));
  return { path, parent: parentOf(path), entries, roots };
}
async function isReadableDirectory(path, deps) {
  try {
...
function describeFsError(error, path) {
...
function admit(req, res, reject, log2, label) {
  let rejection;
  if (reject === void 0) {
    log2?.(`[im-gateway] ${label} refused: connection trust service unavailable (failing closed)`);
    rejection = 401;
  } else {
    try {
      rejection = reject(req);
    } catch (error) {
      log2?.(`[im-gateway] ${label} refused: trust check failed: ${String(error)}`);
      rejection = 401;
    }
  }
  if (rejection !== void 0) {
  ...
  return async (req, res) => {
    ...
    try {
      ...
      writeJson(
        res,
        500,
        { path: "", parent: null, entries: [], roots: [], error: "browse unavailable" },
        req.method === "HEAD"
      );
    }
  };
...
function writeJson(res, status, body, head) {
  ...
  res.end(head ? void 0 : JSON.stringify(body));
}
...
function apply(ctx, config) {
  ...
  ctx.inject(["webServer"], (webCtx) => {
    const reject = (req) => {
      const gate = webCtx.get("connection");
      if (gate === void 0) {
    ...
    const log2 = (message) => webCtx.logger.warn(message);
  ...
  ctx.effect(() => {
    ...
    const boot = inbound.listen().then(() => {
      ...
      ctx.logger.error(
        `[im-gateway] inbound webhook FAILED to bind ${config.host}:${config.port}: ${String(error)} \u2014 IM channels keep running, but the inbound webhook is unavailable`
      );
    });
    return async () => {
      await boot;
