...
function stateFileFor2(channelId) {
  ...
  return join4(homedir4(), ".dsh", "im-workspace", "wechat-state", `${key}.json`);
}
...
var DEFAULT_BASE_URL, CHANNEL_VERSION, BOT_TYPE, POLL_INTERVAL_MS, HTTP_TIMEOUT_MS, QR_RETRY_INTERVAL_MS, WechatIlinkTransport;
var init_wechat = __esm({
  "src/transports/wechat.ts"() {
    ...
    WechatIlinkTransport = class {
      ...
      async saveState() {
        try {
          await mkdir3(join4(homedir4(), ".dsh", "im-workspace", "wechat-state"), { recursive: true });
