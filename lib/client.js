...
function ChannelsSection(props) {
  ...
  const statusOf = (ch) => liveStatus ? status[ch.id]?.status ?? "idle" : "configured";
  const statusLabel = (s) => {
    switch (s) {
      case "configured":
  ...
  const activeStatusKey = liveStatus ? active ? statusOf(active) : creating ? "connecting" : "idle" : "configured";
